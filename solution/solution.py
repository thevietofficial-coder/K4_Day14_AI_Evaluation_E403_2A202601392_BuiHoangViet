...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}\n\n"
            "Return valid JSON. Use either a direct criterion-to-score object "
            "or an object with 'scores' and optional 'reasoning' fields."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        pairs = list(zip(averages[0::2], averages[1::2]))
        if len(pairs) >= 2:
            positional_bias = all(first > second for first, second in pairs)
...
class BenchmarkRunner:
    ...
    def identify_failures(
        ...
        return [
            ...
            if any(
                score < threshold
                for score in (r.faithfulness, r.relevance, r.completeness)
            )
        ]
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            rows.append(
                f"| F{index:03d} | {_cell(failure_type)} | "
                f"{_cell(root_cause)} | {_cell(suggestion)} | Open |"
            )
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        actions = {
            "hallucination": (
                "Add grounding checks that reject claims unsupported by the "
                "retrieved context"
            ),
            "irrelevant": (
                "Improve intent detection and add prompt examples for answering "
                "the user's exact question"
            ),
            "incomplete": (
                "Improve retrieval coverage and require answers to include all "
                "conditions and exceptions"
            ),
            "off_topic": (
                "Add query routing and scope checks before retrieval and generation"
            ),
            "refusal": (
                "Calibrate guardrails with allowed-request examples to reduce "
                "unnecessary refusals"
            ),
            "unknown": (
                "Inspect uncategorized traces and assign a consistent failure type"
            ),
        }
        suggestions = [
        ...
        fallback_actions = [
            "Add representative failed cases to the regression evaluation suite",
            "Review retrieved chunks and generation traces before changing prompts",
            "Track per-metric averages after each fix to verify the improvement",
        ]
